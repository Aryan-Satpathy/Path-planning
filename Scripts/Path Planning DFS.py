...
def DFS(start, goal, navigable, img, costUpdation = False) : 
    lastTime = time.time()
    openSetPos = [start]
    openSetCost = [0]
    closedList = {}
    ...
    while len(openSetPos) : 
        ...
        for n in neighbours : 
            ...
            if neighbour not in openSetPos and neighbour not in closedList : 
                ...
                openSetCost = [tentativecost] + openSetCost
...
# print(end - start)
start = time.time()
DFSPath = DFS(Start, Destination, navigable, img4, costUpdation = False)
end = time.time()
print(end - start)
cv2.waitKey(0)
cv2.destroyAllWindows()
